...
function initUserInterface_inputBookList() {
    ...
    for (let inputBookNum = etApp.Workbooks.Count; inputBookNum >= 1; inputBookNum--) {
        ...
        if (checkInputBook(inputBook) !== true) {
            checkboxNode.setAttribute("disabled", "");
            labelNode.setAttribute("style", "color: gray;");
        }
        // else {
        //     // 如果检查通过，且此工作簿是活动工作簿，默认勾选
        //     if (etApp.ActiveWorkbook.Name === inputBook.Name) {
 ...
 * @returns 检查结果。检查通过时，返回 true: boolean
 */
function checkInputBook(inputBook) {
    ...
    for (const inputDataTitle of inputDataTitleList) {
        if (inputSheet.Rows.Item(1).Find(inputDataTitle, undefined, -4163, 1) === null) {
            return false;
        }
    }
    ...
    // 检查输入工作表中的输入数据
    if (checkInputData(inputSheet) !== "pass") {
        return false;
    }
...
function initUserInterface_outputSheetSettings() {
    ...
    // 由于部分数据为计算结果，依赖原始数据完整性，当勾选【任意“删除列数据”】时，自动勾选【输出数据转换为值】，先全部转换为值，取消依赖，再删除
    // 所以当取消勾选【输出数据转换为值】时，自动取消勾选【全部“删除列数据”】

    // 当指定【学分标准版本】时，版本与【入学年份】无关，故【入学年份】列数据不再计算，用“-”占位，且此列数据需要删除

    // 故当指定【学分标准版本】时，自动勾选【删除入学年份列数据】；又因为勾选了【任意“删除列数据”】，自动勾选【输出数据转换为值】
    // 当取消勾选【删除入学年份列数据】时，自动将【学分标准版本】调整为“自动匹配”

    // 而当取消勾选【输出数据转换为值】时，自动取消勾选【全部“删除列数据”】，包括【删除入学年份列数据】，此时自动将【学分标准版本】调整为“自动匹配”
    ...
    creditStandredVersion.addEventListener("change", () => {
        switch (creditStandredVersion.value) {
            ...
            case "standred-version-2023":
                ...
                // 手动触发一次【删除入学年份列数据】的change事件，勾选【输出数据存储为值】
    ...
    convertOutputDataIntoValues.addEventListener("change", () => {
        if (convertOutputDataIntoValues.checked === false) {
            ...
            // 手动触发一次【删除入学年份列数据】的change事件，将【学分标准版本】修改为自动匹配
            deleteEnrollmentYearColumn.dispatchEvent(new Event("change"));
        }
    });
...
function initUserInterface_otherHtmlElements() {
    ...
    const noValidInputFiles = document.getElementById("no-valid-input-files");
    ...
    hideInvalidInputFiles.addEventListener("click", () => {
        ...
        if (document.querySelectorAll(`input[name="input-book"]:not([disabled])`).length === 0) {
            noValidInputFiles.removeAttribute("hidden");
        }
    });
...
function main() {
    try {
        ...
        let checkResult = checkInputData(tempSheet);
        if (checkResult !== "pass") {
...
function organizeInputSheet(inputSheet) {
    ...
    for (let i = inputDataTitleList.length - 1; i >= 0; i--) {
        ...
        targetColumnNum = inputSheet.Rows.Item(1).Find(inputDataTitleList[i], undefined, -4163, 1).Column;
...
/**
 * 检查输入数据
 * @param {Et.Worksheet} inputSheet 临时工作表
 * @returns 
 */
function checkInputData(inputSheet) {
    ...
    // 检查所有输入数据中，是否存在重复的学号
    targetColumnNum = inputSheet.Rows.Item(1).Find("学号", undefined, -4163, 1).Column;
    ...
    formulaStr = `COUNTA(UNIQUE('[${inputBookName}]${inputSheetName}'!${targetRangeAddr}))`;
    if (etApp.Evaluate(formulaStr) !== inputRowNumMax - 1) {
        checkResult = "输入数据中，存在学号相同的重复数据行。";
        return checkResult;
    }

    // 检查所有输入数据中，参与开始时间是否唯一
    targetColumnNum = inputSheet.Rows.Item(1).Find("参与开始时间", undefined, -4163, 1).Column;
    targetStartCell = inputSheet.Cells.Item(2, targetColumnNum);
    targetEndCell = inputSheet.Cells.Item(inputRowNumMax, targetColumnNum);
    targetRangeAddr = inputSheet.Range(targetStartCell, targetEndCell).Address(false, false);
    formulaStr = `COUNTA(UNIQUE('[${inputBookName}]${inputSheetName}'!${targetRangeAddr}))`;
    if (etApp.Evaluate(formulaStr) !== 1) {
        checkResult = "输入数据中，参与开始时间不唯一。";
        return checkResult;
    }

    // 检查所有输入数据中，参与结束时间是否唯一
    targetColumnNum = inputSheet.Rows.Item(1).Find("参与结束时间", undefined, -4163, 1).Column;
    targetStartCell = inputSheet.Cells.Item(2, targetColumnNum);
    targetEndCell = inputSheet.Cells.Item(inputRowNumMax, targetColumnNum);
    targetRangeAddr = inputSheet.Range(targetStartCell, targetEndCell).Address(false, false);
    formulaStr = `COUNTA(UNIQUE('[${inputBookName}]${inputSheetName}'!${targetRangeAddr}))`;
    if (etApp.Evaluate(formulaStr) !== 1) {
        checkResult = "输入数据中，参与结束时间不唯一。";
        return checkResult;
    }
    ...
    if (document.getElementById("calculate-annual-average-valid-credits").checked) {
        ...
        formulaStr = `MONTH('[${inputBookName}]${inputSheetName}'!${targetRangeAddr})`;
        if (etApp.Evaluate(formulaStr) < 6 || etApp.Evaluate(formulaStr) > 9) {
            checkResult = "输入数据中，参与结束时间应落在6-9月。";
            return checkResult;
        }
    }
...
function organizeOutputSheet(outputSheet) {
    ...
    if (document.getElementById("delete-not-good-result-rows").checked) {
        ...
        let targetStartCell = outputSheet.Range("Z:Z").Find("NG", undefined, -4163, 1);
